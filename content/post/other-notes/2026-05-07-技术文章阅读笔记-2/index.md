...
5. 2017年<< Attention Is All You Need >>提出了Transformer框架.
6. 2018年<< Improving Language Understanding
by Generative Pre-Training >>提出了基于Transformer的GPT框架
7. 2023年<< LLaMA: Open and Efficient Foundation Language Models >>提出了基于Transformer的LLaMA框架.


- 这些主要论文中间穿插着许多奠基者的研究成果,我也会适当地学习这些论文.

>而由于我只是一个业余爱好者,只想准确的了解现代大模型背后的原理,所以只会挑选最经典或者最优秀的论文来大致学习一下.
### 卷积神经网络是如何实现反向传播计算的
...
![alt text](PixPin_2026-05-20_20-12-52.webp)
...
![alt text](PixPin_2026-05-20_18-40-08.webp)

### 总结
尽管这篇论文很经典,但论述实在太专业了,而且理论公式让人看着头疼:
![alt text](PixPin_2026-05-20_18-48-31.webp)


简单来说就是,LSTM将RNN中的隐藏层单元替换成了LSTM单元(Cell);
![alt text](PixPin_2026-05-20_18-58-17.webp)

这个单元可以进行循环计算,能够将不同时刻的数据联系起来:
![alt text](PixPin_2026-05-20_19-30-50.webp)

这种架构能够大大加速训练过程,并减少时间间隔过长带来的信息丢失.
...
>由于MIPS是按字编址的,但存储器中的数据是按字节编址的,所以加上偏移量比如说8时,实际上是加上了4x8个字节,这样才能正确读到A[8],而不会错读到A[8/4].


### 基本操作数

1. add(加法)/sub(减法)指令后一般跟三个寄存器,第一个寄存器存储计算结果,后两个为要参与运算的寄存器
2. load指令用来装载从存储器复制到寄存器的数据,使用`lw`符号,表示`load word`
3. store指令用来将数据从寄存器复制到寄存器中,使用`sw`符号,表示`store word`.
...
R型,用于普通的寄存器运算指令:
![alt text](PixPin_2026-05-21_14-14-31.webp)

I型,用于立即数和数据传送指令:
![alt text](PixPin_2026-05-21_14-16-10.webp)
