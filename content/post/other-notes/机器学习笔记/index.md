...
### 核心概念
神经网络是一种模仿生物大脑处理信息方式的数学计算模型。其本质是一个**非线性函数逼近器**。

* **基本结构**：由大量简单的处理单元——**神经元 (Neurons)** 互联而成。
    * **输入层 (Input Layer)**：接收原始数据。
    * **隐藏层 (Hidden Layers)**：通过权重 (Weights) 和偏置 (Biases) 对数据进行特征提取和变换。
    * **输出层 (Output Layer)**：产生最终预测结果（如分类标签或数值）。

### 神经网络的历史
* **起源期 (1940s-1960s)**：
    * 1943年：M-P模型建立数学基础。
    * 1958年：**感知机 (Perceptron)** 诞生，神经网络实现硬件化。
* **沉寂期 (1970s)**：由于无法解决异或 (XOR) 等线性不可分问题，神经网络进入第一个“AI寒冬”。
* **复兴期 (1980s-1990s)**：
    * 1986年：**反向传播算法 (BP)** 被重新推广，多层网络训练成为可能。
    * 1989年：LeCun 提出卷积神经网络 (CNN) 并在手写识别上取得成功。
* **爆发期 (2006年-2012年)**：
    * 2006年：Hinton 提出深度信念网络，标志着“深度学习”时代的到来。
    * 2012年：AlexNet 席卷 ImageNet，证明了深度网络加 GPU 加速的绝对优势。
* **大模型时代 (2017年至今)**：
    * 2017年：**Transformer** 架构问世，通过自注意力机制彻底改变了序列建模。
    * 2022年后：以 GPT 为代表的大语言模型 (LLM) 实现了跨任务的通用智能。

## 神经网络分类

神经网络根据其拓扑结构和数据处理方式的不同，主要分为以下五类：

### 前馈神经网络 (FNN)
* **特征**：数据单向流动，层与层之间全连接。
* **应用**：结构化数据分类、简单的数值回归任务。

### 卷积神经网络 (CNN)
* **特征**：引入卷积层和池化层，具有**局部感知**和**参数共享**的特性。
* **应用**：计算机视觉（图像识别、目标检测、医疗影像分析）。

### 循环神经网络 (RNN)
* **特征**：神经元之间存在环路，允许信息持久化，具有“记忆”能力。
* **衍生**：LSTM (长短期记忆网络) 和 GRU，解决了传统 RNN 的梯度消失问题。
* **应用**：序列数据处理（语音识别、机器翻译、时间序列预测）。

### 生成模型 (GANs & Diffusion)
* **特征**：生成对抗网络 (GAN) 通过判别器与生成器的博弈来创造新数据。
* **应用**：图像生成、深度伪造 (Deepfake)、艺术创作。

### 变换器架构 (Transformer)
* **特征**：完全抛弃循环结构，依赖**注意力机制 (Attention)** 并行处理数据。
* **应用**：自然语言处理 (NLP) 的核心（GPT, BERT）、视觉领域的 ViT。


# Pytorch学习
根据我先前发布的uv笔记,可以很轻松的用uv搭建一个pytorch的学习环境,完美避开Anaconda这一个毒瘤.
