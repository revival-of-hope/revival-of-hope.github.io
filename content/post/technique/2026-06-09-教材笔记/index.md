...
两个码字不相同的位数称为海明距离,为了可靠地检测一串码字中的d个错误,我们可以构建一个彼此之间的海明距离为d+1的合法码字字典,也就是说,我们只设置一部分的有效码字,至于其他的码字都可以根据海明距离来判断它是由哪个合法码字出错后转变成的.
...
### 滑动窗口协议
## 介质访问控制(MAC,Medium Access Control)子层(待补充)
### 信道分配概览
...
#### 限定词
在`select`后面可以加上`all`和`distinct`,分别用来声明保留重复项和去除重复项,其中all是默认值可以不写.
## ch6: 关系代数
