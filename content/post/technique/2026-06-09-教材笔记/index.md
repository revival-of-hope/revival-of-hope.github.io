...
第二个方案是非坚持CSMA(nonpersistent CSMA),站点在发现信道正在使用时,并不会一直守着,而是等待一段随机的时间后再来侦听,尽管这可以降低冲突,但在信道不是那么繁忙时会降低信道的利用率.
...
更值得说明的是,站点要发送帧前就必须等待一段时间,无论信道是否空闲,等倒计时结束后再发送帧,从而尽可能避免冲突.
...
### ch1: 概览
>**操作系统**是计算机系统中的一个系统软件，管理和控制计算机系统的软、硬件资源，合理地组织计算机的工作流程，以便有效利用这些资源为用户提供一个功能强、使用方便的工作环境，从而在计算机与用户之间起到接口的作用。
